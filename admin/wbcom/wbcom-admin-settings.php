...
if ( !class_exists( 'Wbcom_Admin_Settings' ) ) {
	...
	class Wbcom_Admin_Settings {
	    ...
	    public function show_notices() {
	        ...
	        $wb_json_request_url           = WB_NOTICE_JSON_URL . 'bp-activity-social-share/notice.json';
	        $plugin_request_url            = BP_ACTIVITY_SHARE_PLUGIN_URL . 'admin/wbcom/assets/json/notice.json';
			...
			if ( empty( $response_promotions ) ) {
				...
				if ( ! is_wp_error( $response_plugin_request ) ) {
					if ( isset( $response_plugin_request['response']['code'] ) && ( $response_plugin_request['response']['code'] == 200 ) ) {
						$response_promotions = isset( $response_plugin_request['body'] ) ? $response_plugin_request['body'] : '';
					}
				}
			}

			if ( ! empty( $response_promotions ) ) {
				$response_promotions = json_decode( $response_promotions, true );
				if( !empty( $response_promotions ) && is_array( $response_promotions ) ) {
					foreach ( $response_promotions as $key => $notice ) {
						$screen = get_current_screen();
						if ( 'all' == $notice['display'] || $screen->base == $notice['display'] ) {
							$display = true;
						} else {
							$display = false;
						}
						...
						if ( $display ) {
							if ( 'banner' === $notice['type'] ) {
							?>
					            <div class="notice wbcom-promotional-banner-notice is-dismissible">
				                	<a href="<?php echo esc_url( $notice['link'] ); ?>">
				                    	<img src="<?php echo esc_url( $notice['thumbnail'] ); ?>" alt="<?php echo esc_attr( $notice['title'] ); ?>" style="">
				                    </a>
					            </div>
							<?php
							} else { ?>
								<div class="notice wbcom-promotional-content-notice is-dismissible">
					                ...
					                <div class="content" style="">
					                    ...
					                    <a href="<?php echo esc_url( $notice['link'] ); ?>" class="button button-primary" target="_blank"><?php echo !empty( $notice['btn_text'] ) ? esc_html( $notice['btn_text'] ) : esc_html__( 'Learn More &rarr;', 'buddypress-share' ); ?></a>
					                </div>
					                <div class="clear"></div>
					            </div>				            
							<?php }
